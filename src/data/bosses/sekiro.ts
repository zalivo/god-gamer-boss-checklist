import { GameData as Game } from "@/utils/models";

export const Sekiro: Game = {
    title: "Sekiro",
    bosses: [
        {
            name: "Leader Shigenori Yamauchi",
            killed: false
        },
        {
            name: "Genichiro Ashina",
            killed: false
        },
        {
            name: "General Naomori Kawarada",
            killed: false
        },
        {
            name: "Chained Ogre",
            killed: false
        },
        {
            name: "General Tenzen Yamauchi",
            killed: false
        },
        {
            name: "Shinobi Hunter Enshin of Misen",
            killed: false
        },
        {
            name: "Juzou the Drunkard",
            killed: false
        },
        {
            name: "Gyoubu Oniwa",
            killed: false
        },
        {
            name: "Lady Butterfly",
            killed: false
        },
        {
            name: "Blazing Bull",
            killed: false
        },
        {
            name: "Lone Shadow Longswordsman",
            killed: false
        },
        {
            name: "Seven Ashina Spears - Shikibu Toshikatsu Yamauchi",
            killed: false
        },
        {
            name: "General Kuranosuke Matsumoto",
            killed: false
        },
        {
            name: "Ashina Elite - Jinsuke Saze",
            killed: false
        },
        {
            name: "Genichiro Ashina, Way of Tomoe - Ashina Castle",
            killed: false
        },
        {
            name: "Armored Warrior",
            killed: false
        },
        {
            name: "Long-arm Centipede Sen'un",
            killed: false
        },
        {
            name: "Folding Screen Monkeys",
            killed: false
        },
        {
            name: "Long-arm Centipede Giraffe",
            killed: false
        },
        {
            name: "Guardian Ape",
            killed: false
        },
        {
            name: "Snake Eyes Shirahagi",
            killed: false
        },
        {
            name: "Guardian Ape (Headless)",
            killed: false
        },
        {
            name: "Tokujiro the Glutton",
            killed: false
        },
        {
            name: "Mist Noble",
            killed: false
        },
        {
            name: "Snake Eyes Shirafuji",
            killed: false
        },
        {
            name: "O'Rin of the Water",
            killed: false
        },
        {
            name: "Corrupted Monk (Mibu Village)",
            killed: false
        },
        {
            name: "Headless (Old Grave) Underwater",
            killed: false
        },
        {
            name: "Shichimen Warrior (Bottomless Hole)",
            killed: false
        },
        {
            name: "Great Serpent - All Encounters",
            killed: false
        },
        {
            name: "Shichimen Warrior (Guardian Ape's Burrow)",
            killed: false
        },
        {
            name: "Chained Ogre (Castle)",
            killed: false
        },
        {
            name: "Lone Shadow Masanaga the Spear-Bearer",
            killed: false
        },
        {
            name: "Lone Shadow Vilehand",
            killed: false
        },
        {
            name: "Headless (Bell Demon's Temple)",
            killed: false
        },
        {
            name: "Headless (Hidden Forest)",
            killed: false
        },
        {
            name: "Headless (Under-Shrine Valley)",
            killed: false
        },
        {
            name: "Emma, the Gentle Blade",
            killed: false
        },
        {
            name: "Isshin Ashina",
            killed: false
        },
        {
            name: "Great Shinobi - Owl",
            killed: false
        },
        {
            name: "Corrupted Monk (Fountainhead Palace)",
            killed: false
        },
        {
            name: "Sakura Bull of the Palace",
            killed: false
        },
        {
            name: "Okami Leader Shizu",
            killed: false
        },
        {
            name: "Blue Ogre",
            killed: false
        },
        {
            name: "Headless (Fountainhead Palace) Underwater",
            killed: false
        },
        {
            name: "Shichimen Warrior (Fountainhead Palace)",
            killed: false
        },
        {
            name: "Old Dragons of the Tree",
            killed: false
        },
        {
            name: "Divine Dragon",
            killed: false
        },
        {
            name: "Ashina Elite - Ujinari Mizuo",
            killed: false
        },
        {
            name: "Shigekichi of the Red Guard",
            killed: false
        },
        {
            name: "Great Colored Carp",
            killed: false
        },
        {
            name: "Seven Ashina Spears - Shume Masaji Oniwa",
            killed: false
        },
        {
            name: "Lone Shadow Masanaga the Spear-Bearer - Hirata Estate",
            killed: false
        },
        {
            name: "Juzoe the Drunkard - Hirata Estate",
            killed: false
        },
        {
            name: "Owl (Father) - Hirata Estate ",
            killed: false
        },
        {
            name: "Demon of Hatred ",
            killed: false
        },
        {
            name: "Genichiro and Isshin, the Sword Saint ",
            killed: false
        }
    ]
}